...
from textkb.utils.io import load_adjacency_lists, load_tokenized_concepts, \
...
def main():
    ...
    output_subdir = f"gat_{gat_num_layers}_{gat_num_hidden_channels}_{gat_num_hidden_channels}_{gat_dropout_p}_" \
    ...
    if graph_model_type == "shared_encoder":
        ...
        if freeze_graph_bert_encoder:
            logging.info(f"Using freezed GEBERT's BERT encoder...")
            for param in concept_bert_encoder.parameters():
    ...
    alignment_model = AlignmentModel(sentence_bert_encoder=sentence_bert_encoder,
                                     ...
                                     multigpu=multigpu_flag).to(device)
    ...
    logging.info("Creating artificial sentences data...")
    num_artificial_samples = int(10 ** 7)

    input_ids = tuple(((1,) * random.randint(20, 128) for i in range(num_artificial_samples)))
    token_entity_mask = tuple(((1,) * len(t) for t in input_ids))
    edge_index_token_idx_list = tuple(((0,) * random.randint(5, 30) for i in range(num_artificial_samples)))
    logging.info("edge_index_token_idx_list created...")
    edge_index_entity_idx_list = tuple(((1,) * len(lst) for lst in edge_index_token_idx_list))
    logging.info("edge_index_entity_idx_list created...")
    tr_tokenized_data_dict = {
        "input_ids": input_ids,
    ...
    logging.info("Artificial data created...")
    # tr_tokenized_data_dict = load_tokenized_sentences_data_v2(tokenized_data_dir=train_tokenized_sentences_path)
